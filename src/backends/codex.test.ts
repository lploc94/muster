// Characterization tests for the Codex ACP adapter (`CodexBackend`).
//
// Codex is the closest clone of the canonical Claude adapter: it matches Claude
// on the full FAILURE_STOP_REASONS set, the `usage_update` case, the
// `result.usage` post-turn usage (incl. thoughtTokens) and the mcp/builtin tool
// `kind` classification. The one documented drift is empty-string text chunks:
// Codex emits a `raw` event for them instead of dropping them like Claude.
//
// These tests lock in the CURRENT observable behavior of `run()` — the emitted
// `NormalizedEvent` stream — so the planned dedup refactor into a shared
// `runAcpTurn` can be proven behavior-preserving. They are intentionally
// black-box: the private mapping helpers are exercised only through `run()`,
// which is the seam the refactor must keep stable.
...
import { CodexBackend, codexAgentConfig } from './codex';
...
describe('CodexBackend — identity', () => {
  it('exposes the codex name and full capability set', () => {
...
describe('CodexBackend.run — session + streaming', () => {
  ...
  it('connects (with extraEnv) before opening a session, then passes cwd/prompt through', async () => {
    ...
    await runTurn(new CodexBackend(), options({ input: { kind: 'agent', prompt: 'do it' }, cwd: '/work', extraEnv }), fake, {});
  ...
  it('resumes an existing session via loadSession when resumeId is set', async () => {
    const events = await runTurn(new CodexBackend(), options({ resumeId: 'sess-r' }), fake, {});
...
describe('CodexBackend.run — tool events', () => {
  it('maps tool_call to toolStarted with the codex: id prefix, name, kind and input', async () => {
    const events = await runTurn(new CodexBackend(), options(), fake, {
      updates: [
        {
...
describe('CodexBackend.run — usage', () => {
  ...
  it('emits a usage event from the prompt result (including thoughtTokens) before the terminal', async () => {
    const events = await runTurn(new CodexBackend(), options(), fake, {
      result: {
        stopReason: 'end_turn',
        usage: { totalTokens: 5, thoughtTokens: 2, modelId: 'codex-x' },
        _meta: { trace: 1 },
      },
    });
    const usageIdx = events.findIndex((e) => e.type === 'usage');
...
describe('CodexBackend.run — terminal classification', () => {
  it('end_turn -> turnCompleted with meta.stopReason', async () => {
    const events = await runTurn(new CodexBackend(), options(), fake, { result: { stopReason: 'end_turn' } });
    expect(events.at(-1)).toEqual({ type: 'turnCompleted', meta: { stopReason: 'end_turn' } });
  });
  ...
  it('a non-failure non-end_turn stopReason -> "stopped" error WITH meta', async () => {
    const events = await runTurn(new CodexBackend(), options(), fake, { result: { stopReason: 'surprise' } });
    expect(events.at(-1)).toEqual({
      type: 'error',
...
describe('CodexBackend.run — connection lines', () => {
...
describe('CodexBackend.run — cancellation & errors', () => {
  it('yields only a cancellation terminal when the signal is already aborted', async () => {
    const controller = new AbortController();
    controller.abort();
    const events = await runTurn(new CodexBackend(), options({ signal: controller.signal }), fake);
  ...
  it('reports an unsupported resume when the client cannot load sessions', async () => {
    ...
    H.current = fake;
    const events = await runTurn(new CodexBackend(), options({ resumeId: 'sess-r' }), fake);
    expect(events).toContainEqual({ type: 'error', message: 'Codex agent does not support session resume' });
    expect(fake.calls.loadSession).toHaveLength(0);
  });

  it('wraps a generic prompt rejection with the "Codex ACP error" prefix', async () => {
    const events = await runTurn(new CodexBackend(), options(), fake, { reject: new Error('boom') });
    expect(events.at(-1)).toEqual({ type: 'error', message: 'Codex ACP error: boom' });
  });

  it('passes through an "agent exited" rejection message unwrapped', async () => {
    const events = await runTurn(new CodexBackend(), options(), fake, {
      reject: new Error('Codex agent exited (code 1)'),
    });
    expect(events.at(-1)).toEqual({ type: 'error', message: 'Codex agent exited (code 1)' });
  });
  ...
  it('passes through a "not running" rejection message unwrapped', async () => {
    ...
    expect(events.at(-1)).toEqual({ type: 'error', message: 'agent is not running' });
  });
});

describe('codexAgentConfig.resolveAuth (drift: never throws)', () => {
  it('selects the api-key method when an API key (CODEX_API_KEY) and matching method are present', () => {
    const cfg = codexAgentConfig();
    const result = cfg.resolveAuth!(
      { authMethods: [{ id: 'api-key' }] } as never,
      { CODEX_API_KEY: 'sk-test' } as never,
    );
    expect(result).toEqual({ methodId: 'api-key', meta: { headless: true } });
  });

  it('also honors OPENAI_API_KEY for the api-key branch', () => {
    const cfg = codexAgentConfig();
    const result = cfg.resolveAuth!(
      { authMethods: [{ id: 'api-key' }] } as never,
      { OPENAI_API_KEY: 'sk-test' } as never,
    );
    expect(result).toEqual({ methodId: 'api-key', meta: { headless: true } });
  });

  it('returns null (delegates to Codex cached login) when no API key is configured', () => {
    const cfg = codexAgentConfig();
