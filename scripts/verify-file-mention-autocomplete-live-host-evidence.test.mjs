...
const evidencePath = new URL(
  '../docs/uat/m011-s04/file-mention-autocomplete-live-host-evidence.md',
  import.meta.url,
);
...
function validate(text) {
  ...
  for (const pattern of forbidden) {
    assert.ok(!pattern.test(scenarioEvidence), `Forbidden evidence content: ${pattern}`);
  }
...
test('tracked file-mention autocomplete live-host ledger satisfies the complete contract', async () => {
  let text;
  try {
    text = await readFile(evidencePath, 'utf8');
  } catch (error) {
    assert.fail(
      `Missing file-mention autocomplete live-host evidence ledger (${error.code ?? error.message})`,
    );
  }
...
test('rejects omitted scenarios, invalid verdicts, malformed timestamps, and missing fields', () => {
  ...
  assert.throws(
    () => validate(valid.replace('### FILE-MENTION-POPUP-DISCOVERY', '### OTHER')),
  ...
  assert.throws(
    () => validate(valid.replace('2026-07-15T00:00:00Z', 'yesterday')),
...
test('requires actionable blockers, bounded fields, cleanup, and direct evidence for live verdicts', () => {
  ...
  assert.throws(
    () =>
      validate(
        valid.replace(
          'Attempted: reserve the scenario for live execution. Blocker: live host execution belongs to T02.',
          'host unavailable',
        ),
      ),
  ...
  assert.throws(
    () =>
      validate(
        valid.replace(
          'Observe the named behavior in a live Extension Development Host.',
          'x'.repeat(501),
        ),
      ),
