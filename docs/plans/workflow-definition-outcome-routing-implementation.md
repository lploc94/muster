...
## Target
Refactor Muster's existing workflow definition, package, persistence, and routing implementation in place so its one canonical behavior exactly matches `docs/plans/workflow-definition-v2-design.md`: strict `muster.workflow/v2` manifests, named semantic composition, durable outcome routing and decision repair, and zero/nonzero execute routing, with intentional breaking changes and no parallel legacy model. Host-run authorization is a live resource-scoped gate resolved from the task/workflow execution cwd so the effective workspace-folder policy authorizes or revokes local execution without reload.

## Source Of Truth
- Plan schema: `loop-plan/v1`
- Request/report: `docs/plans/workflow-definition-v2-design.md`, plus the explicit user decisions to refactor the current implementation directly into the design's final behavior (no parallel versioned architecture, no migration or backward compatibility, no new bundled-workflow scope, and no work without a concrete design outcome) and, on 2026-09-01, to make `muster.verification.hostRun` resource-scoped rather than relax the native live-transition proof.
- Baseline commit: `bd2590c`
- Plan path: `docs/plans/workflow-definition-outcome-routing-implementation.md`

## Current State
- Completed: Commit `bd2590c` adds the authoritative design; Phases 1-7 implement the canonical definition, package, persistence, composition, decision-repair, execute-routing, bounded projection, and assembled-journey contracts. Final implementation qualification through `182707c` passes every Global Gate, including exact VS Code 1.135.0 packaged qualification, and the latest complete-plan branch review is APPROVE after its payload-reclamation finding was fixed.
- Remaining: None.
- Worktree: No pre-existing unrelated changes were present. Final review and qualification fixes are isolated in `8ebb88f`, `ffb1d2c`, `716fc91`, `66a5093`, and `182707c`; generated native artifacts remain untracked/ignored rather than entering plan commits.
...
- `muster.verification.hostRun` is contributed with VS Code `scope: resource`; normal VS Code configuration precedence remains authoritative, but the resource is the durable task/workflow execution cwd rather than the active editor or a window-global lookup.
- The engine passes the relevant cwd into the live authorization resolver at workflow start, immediately before script process execution, and at verification settle. A missing, malformed, or throwing resolver fails closed, and changing one folder's effective value must not authorize a task bound to another folder.
- Native qualification on exact VS Code 1.135.0 must exercise one real resource-scoped setting transition from disabled rejection to enabled acceptance without reload, restore the original workspace-folder value, and re-inspect the same resource to prove exact restoration (including `undefined` meaning no override); source inspection, an in-memory override, or two separately seeded hosts is not equivalent proof.
...
| Canonical native acceptance | `MUSTER_VSCODE_VERSION=1.135.0 npm run test:script-workflow-acceptance` | Pass only when the packaged Extension Development Host reports exact VS Code 1.135.0 and observes the live resource-scoped authorization transition, exact post-cleanup restoration, and assembled workflow journey |
...
## Plan Review
- Status: APPROVE
- Rounds: 2 renewed rounds (original review: 3 rounds)
- Open issues: None — renewed Codex issues 1-2 were fixed and re-verified against the resource-scoped native contract.
...
- Current behavior: Phase 7 exposes bounded title and decision state across status, graph, wire, host, and webview paths; its assembled journey proves durable PREV correction, mid-run reload/recovery, two named terminal outputs, and exact downstream composition. `muster.verification.hostRun` is resource-scoped and resolved live from each durable task/workflow cwd at start, pre-spawn, and verification settle. The packaged Extension Development Host qualification passes on exact VS Code 1.135.0 after one real workspace-folder `false -> true` transition without reload and exact observed restoration.
- Code evidence: `src/task/repository.ts:getWorkflowStatusForTask/getWorkflowGraphForTask/workflowGraphNodeProjection`; `src/task/workflow-types.ts:WorkflowTaskStatusProjection/WorkflowGraphProjection`; `src/shared/workflow-graph-wire.ts`; `src/host/workflow-graph.ts`; `src/host/workflow-graph-route.ts`; `webview/src/lib/workflow-graph-view.ts`; `webview/src/components/WorkflowGraphCanvas.svelte` and `WorkflowGraphModal.svelte`; `src/task/m018-s07-workflow-status-projection.test.ts`; `src/task/m024-s04-workflow-graph-projection.test.ts`; `package.json:muster.verification.hostRun`; `src/task/engine.ts:allowHostVerification/resolveAllowHostVerification/prepareWorkflowStart/computeHostVerdictForSettle`; `src/extension.ts:loadConfiguredTaskEngine/registerLiveUatCommands`; `src/host/script-workflow-uat-fixture.ts`; `scripts/script-workflow-native-host.ts`; `scripts/run-script-workflow-native-uat.mjs`; `docs/qa/script-workflow-qa-plan.md`; and `docs/uat/script-workflow-native-evidence.md`.
- Pattern to follow: Extend the existing bounded/redacted status and graph wire, preserving separate workflow status/execution activity/display state axes and the request-correlation/host-validation/webview-store architecture; do not create a second diagnostics path. For resource lookup, follow the existing cwd-aware `muster.taskTypes`/runtime-fallback configuration pattern in `src/extension.ts`, while passing the actual caller/execution/verification cwd through the engine instead of consulting active-editor state.
- Behavioral contract:
  ...
  - `muster.verification.hostRun` is declared `scope: resource`. At each authorization point, the engine supplies the caller/task execution cwd and the host reads `getConfiguration('muster', vscode.Uri.file(cwd))`; the effective value is re-read live and a throwing lookup denies authorization.
  - In multi-root workspaces, a folder-specific `false` denies only tasks/workflows whose cwd resolves to that folder, while another folder's effective `true` may authorize its own tasks. Focus changes must not change an existing task's authorization resource.
  - On exact VS Code 1.135.0, the packaged native fixture updates the disposable workspace folder's real setting from `false` to `true` without reload, observes no run/task materialization while false, completes the canonical assembled journey while true, restores the exact original workspace-folder value in cleanup, and re-inspects the same resource after restoration before emitting PASS.
- Tests first:
  ...
  - Add a package-contract assertion that `muster.verification.hostRun` is boolean, default-false, and resource-scoped. Add engine regressions proving workflow-start, pre-spawn, and verify-settle authorization callbacks receive the relevant task cwd, re-read live values, fail closed on resolver errors, and keep two multi-root folder values isolated.
  - Establish native red evidence from the current packaged host blocker, then require exact VS Code 1.135.0, the `false -> true` no-reload transition, and post-cleanup equality between the captured and re-inspected `workspaceFolderValue` to produce PASS. Equality must distinguish `undefined` (override absent) from `false`; a fixture that hardcodes policy booleans, trusts a restoration call without re-inspection, or runs disabled/enabled checks in separate hosts must fail the evidence contract.
- Anti-shortcut coverage:
  ...
  - Seed opposite effective host-run values for two cwd resources and authorize in both call orders so a resolver that ignores cwd, caches a window value, or follows the active editor fails. Native evidence must come from the packaged extension on VS Code 1.135.0 after one real in-host setting transition and exact observed restoration, not dependency injection, static profile seeding, or a successful-but-unverified cleanup call.
- Implementation obligations:
  ...
  - Change the engine authorization resolver contract to accept the relevant cwd and thread that cwd through workflow-start policy, script pre-spawn authorization, and verification settle without deriving it from window focus. Preserve static boolean injection for existing deterministic tests.
  - Declare `muster.verification.hostRun` as resource-scoped, bind the extension resolver to the supplied cwd URI, and keep ordinary VS Code user/workspace/workspace-folder precedence. Update the UAT owner to capture `workspaceFolderValue`, update with `ConfigurationTarget.WorkspaceFolder`, restore the captured value, then re-inspect the same URI and return a redacted `workspaceFolderValueRestored: true` observation only when strict equality succeeds (including `undefined`); do not add a production settings UI or test-only bypass.
  - Pin the canonical native runner/gate to VS Code 1.135.0, reject a host result whose `vscodeVersion` differs, and make the QA/evidence verifier require that exact version plus the restoration boolean. Other VS Code versions may be diagnostic-only and cannot qualify AC-3 or AC-5.
  - Update current settings/workflow/QA/UAT documentation to state the resource and multi-root semantics, live revocation behavior, and truthful native evidence status.
- Acceptance criteria:
  - [x] AC-1: Status/graph/webview truthfully distinguish PREV correction, decision repair attempt N of 3, execution, completion, and final failure without hidden nodes or sensitive leakage - proven by projection/wire/view/Playwright tests.
  - [x] AC-2: Catalog, public docs, storage docs, QA/UAT, and current examples describe only the canonical workflow contract and schema 7 reset behavior - proven by catalog/doc/evidence tests and source inspection.
  - [x] AC-3: The assembled package -> named input -> correction/repair -> named terminal output -> downstream composition journey passes across reload with opaque bounded projections on exact VS Code 1.135.0 after one real packaged-host resource-scoped `false -> true` authorization transition without reload and exact observed restoration - proven by canonical workflow, package, graph, and native UAT tests.
  - [x] AC-4: All repository-wide type, static, build, unit, source-boundary, and webview regression gates pass on final HEAD - proven by the Global Gates.
  - [x] AC-5: Host-run authorization is default-false, resource-scoped, live, cwd-correct at workflow start/pre-spawn/verify settle, fail-closed, and isolated across multi-root folder values; exact VS Code 1.135.0 native evidence also proves restoration by post-cleanup re-inspection, including the original-`undefined` case - proven by package-contract, engine policy, native UAT, and documentation-verifier tests.
- Focused verification:
  - `npx vitest run src/task/m018-s07-workflow-status-projection.test.ts src/task/m018-s08-disposition-claims.test.ts src/task/m024-s04-workflow-graph-projection.test.ts src/host/workflow-graph.test.ts src/host/workflow-graph-route.test.ts src/shared/workflow-graph-wire.test.ts webview/src/lib/workflow-graph-view.test.ts webview/src/lib/workflow-graph-layout.test.ts src/task/workflow-graph-webview-wiring.test.ts src/host/workflow-catalog-route.test.ts src/shared/workflow-catalog-wire.test.ts src/task/script-workflow.test.ts src/task/engine-repository.test.ts`
  - `node --test scripts/verify-script-workflow-qa.test.mjs`
- Phase gates:
  - `npm run test:m024-s05 && npm run test:m024-s06-gate && MUSTER_VSCODE_VERSION=1.135.0 npm run test:script-workflow-acceptance && npm run test:sqlite-storage-docs && npx tsc -p . --noEmit && npm run check:svelte && npm run compile && npm test && npm run test:webview && npm run test:source-boundary && npm run test:source-boundary:fixtures`
- Review: run `codex-impl-review` against this phase and this plan; verdict must be APPROVE.
...
| Final qualification | complete | `8ebb88f`, `ffb1d2c`, `716fc91`, `66a5093`, `182707c`, and this plan-finalization commit | M024 S05: 5/5 plus 2/2 focused Playwright; M024 S06: 15/15 probe, 9/9 assembly, 4/4 evidence; script-workflow QA: 162/162; exact VS Code 1.135.0 native PASS (`catalog=5`, `policy=3`, `runtime=15`) with live resource transition and exact restoration; SQLite docs: 3/3; `npx tsc -p . --noEmit`; Svelte 0 errors/0 warnings; compile; full Vitest 2930/2930; Playwright 92/92; affected reclamation/repair/schema matrix 73/73; repeated keyboard flows 6/6; source-boundary 24 checks, repository boundary, and fixtures 46/46 passed | Complete-plan branch reviews: `bd2590c...ffb1d2c` APPROVE in 3 rounds with 9 findings fixed, none disputed (session `codex-impl-review-20260901-002`), then final `bd2590c...182707c` APPROVE in 2 rounds with 1 payload-reclamation finding fixed, none disputed (session `codex-impl-review-20260901-004`); both sessions finalized and stopped |
