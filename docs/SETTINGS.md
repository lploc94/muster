...
`muster.verification.hostRun` is a default-false, resource-scoped authorization setting and a reserved destination in Execution. Native VS Code Settings remains its only edit path until a dedicated host contract exists; no disabled or `Coming soon` child is rendered in Muster Settings. At workflow start, immediately before a script spawn, and when a verification turn settles, the host re-reads the effective value for that task's durable cwd resource. In a multi-root workspace, a folder override affects tasks in that folder without following active-editor focus or authorizing a sibling folder. Turning the effective value off revokes the next eligible local execution without reload; turning it on still cannot bypass workspace trust or task/workflow opt-in. See the [Verification staging and host contract](SETTINGS-DESIGN.md#verification-staging-and-host-contract) section in the design doc for the future 4-message contract (`requestVerificationSettings` / `verificationSettingsSnapshot` / `updateVerificationSettings` / `verificationSettingsUpdateResult`) and its fail-closed guarantees.
...
- **Host verification follows the task resource.** `muster.verification.hostRun` is resource-scoped and resolved from the task/workflow execution cwd at every authorization point. Normal VS Code user/workspace/workspace-folder precedence applies to that resource; changing editor focus never changes an existing task's resource. Missing, malformed, or failed lookups deny local execution.
...
- **Ship defaults include breakdown and exploration.** Muster package defaults for `muster.taskTypes` ship `coordinate`, `plan`, `breakdown`, `explore`, `implement`, `verify`, and `research` without model pins. Reset in the panel restores those package defaults.
