...
- At least one real settings group is backed by VS Code contributed configuration. Today the retention group exposes `muster.retention.maxTurnsPerTask` and `muster.retention.maxStoredOutputChars` through the extension manifest so VS Code owns default values, minimums, and the user-facing Settings entry. Task-type presets use resource-scoped `muster.taskTypes` (object map) with **Muster ship defaults** (coordinate/plan/implement/verify/research, no model pins); the custom Settings panel can edit and **Save** the full map into workspace settings.json. Explicit empty map fails closed for coordinator create/delegate (see `docs/TASK-MANAGEMENT.md` §8).
